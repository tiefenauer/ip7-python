...
class PreprocessedData(object):
    def __enter__(self):
        ...
        cursor.execute("SELECT count(*) AS num_rows FROM labeled_text_preprocessed")
    ...
    def __iter__(self):
        ...
        cursor.execute("SELECT id, fetchflow_id, content FROM labeled_text_preprocessed")
        for row in cursor:
            yield row
    ...
    def insert(self, fetchflow_id, data):
        ...
        cursor = self.db_fetchflow.cursor()
        cursor.execute("""INSERT INTO labeled_text_preprocessed(fetchflow_id, content) VALUES(%s, %s)""",
                       (fetchflow_id, content))
