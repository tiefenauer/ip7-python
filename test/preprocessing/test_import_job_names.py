...
class TestImportJobNames(unittest.TestCase):
    def test_merge_both_same_returns_actual(self):
        ...
        assert_that(result, is_((actual, 'predicted')))
    ...
    def test_merge_partial_match_returns_full(self):
        ...
        assert_that(testee.merge('Bauspengler', 'Spengler'), is_(('Bauspengler', 'compound')))
    ...
    def test_marge_no_match_actual_is_known_job_returns_actual(self):
        ...
        for actual_variant, prediction_variant in zip(polymechaniker_variants, elektroniker_variants):
            assert_that(testee.merge(actual_variant, prediction_variant), is_((actual_job_name, 'guessed')))

    def test_merge_no_match_actual_is_one_word_returns_actual(self):
        assert_that(testee.merge('Fassadenisolateur', 'Gipser'), is_(('Fassadenisolateur', 'guessed')))
        assert_that(testee.merge('Fassadenisolateur (m/w)', 'Gipser'), is_(('Fassadenisolateur', 'guessed')))
